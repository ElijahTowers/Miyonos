...
tracks, while a direct source open lets Sonos load it lazily. Opening this
specific source type has a bounded 20-second read deadline rather than the
normal short interactive deadline, because a speaker may need a few seconds
to accept a very large music-service collection before it replies.
